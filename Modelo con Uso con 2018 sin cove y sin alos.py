...
archivo_resultante = "C:\\Users\\lepid\\Documents\\Los libertadores\\Proyecto_Articulo_EstadisticaAplicada\\Integrado_sin_al_cov._mod.csv"
...
# Ahora puedes usar este 'modelo' en el código del gráfico de regresión 3D

# Gráfico de Regresión 3D
fig = plt.figure(figsize=(10, 8))
ax = fig.add_subplot(111, projection='3d')
ax.scatter(data['idx_count_60-69a'], data['idx_count_secun18'], data['idx_count_desempl'], c=data['TLoss2kAvg'], cmap='turbo')
ax.set_xlabel('idx_count_70-79a')
ax.set_ylabel('idx_count_secun18')
ax.set_zlabel('idx_count_desempl')
ax.set_title('Gráfico de Regresión 3D')

# Agregar la línea de regresión
x_pred = data[['idx_count_70-79a', 'idx_count_secun18', 'idx_count_desempl']]
y_pred = modelo.predict(x_pred)
ax.plot(data['idx_count_70-79a'], data['idx_count_secun18'], y_pred, color='green',alpha=0.3)
plt.show()
...
modelo_rf = create_model('rf', n_estimators=14)  # Reemplaza 10 con tu valor inicial deseado
...
modelo_rf = create_model('rf', max_depth=14)  # Reemplaza 10 con tu valor inicial deseado
...
# Crea un modelo de Random Forest para regresión y ajusta sus hiperparámetros
modelo_rf_R2 = create_model('rf', n_estimators=20, max_depth=8)  # Ejemplo de ajuste de hiperparámetros
...
plot_model(modelo_rf_R2, plot='feature')
...
# Obtener curvas de aprendizaje
train_sizes, train_scores, test_scores = learning_curve(modelo_rf_R2, X_train, y_train, cv=2)
...
# Crea un modelo de Random Forest para regresión y ajusta sus hiperparámetros
modelo_lightgbm_R2 = create_model('lightgbm', n_estimators=146, max_depth=7)  # Ejemplo de ajuste de hiperparámetros
...
# Obtener curvas de aprendizaje
train_sizes, train_scores, test_scores = learning_curve(modelo_lightgbm_R2, X_train, y_train, cv=5)
...
general_bar_label = 'Positivo' if general_bar_color == 'green' else 'Negativo'
